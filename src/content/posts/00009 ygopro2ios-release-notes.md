...
3. 新增禁用中央阶段栏点击选项，减少误触。  
4. 优化卡片详情栏宽度调节和文字滚动，改进刘海屏下的说明文字、搜索结果及滚动条适配。  
5. 优化设置窗口布局，改善小屏幕和自定义分辨率下的显示与操作。  
6. 新增自定义快速建房按钮和联机帮助，支持保存常用房间号，并修正超先行 2 服地址。  
7. 优化卡图、场地图和列表的内存管理，减少重复加载与资源占用，改善长时间浏览体验。  
8. 调整资源下载线路，优化分块下载、慢速网络超时及下载停滞后的重试处理。  
9. 修复延迟观战时，旧对局结算可能显示或操作下一局录像的问题。  
10. 更新内置卡片资料至 2026 年 9 月 1 日版本。  

首次启动或更新后可能需要同步卡牌资料，请保持网络连接。   
建议更新前备份自己的 ygopro2 文件夹。  

其实本次才是正式的9月的更新，上一个是CDN风控后的紧急更新。  
提前祝大家中秋国庆快乐，玩得开心！  
...
- **更新内容**：  
本次更新聚焦于文件互通、下载速度优化和移动端操作体验。  
  1. 新增 .ydk 牌组和 .yrp3d 录像的导入、分享功能。  
  2. 新增先行卡批量转正式卡功能。  
  3. 优化提高资源下载速度（包括先行卡下载速度、卡片资料下载速度等）。  
  4. 新增决斗可选卡高亮，优化计数物选择提示。  
  5. 优化种族、属性和卡名宣言窗口的触控布局。  
  6. 优化刘海屏、横屏翻转及牌组预览界面适配。  
  7. 更新内置卡片资料至 2608。  
